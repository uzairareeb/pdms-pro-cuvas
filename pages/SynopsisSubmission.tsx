...
const SynopsisSubmission: React.FC = () => {
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterDegree, filterDept, viewFilter]);
      ...
      <div className="space-y-6 lg:space-y-0">
            ...
            <table className="w-full text-left text-sm border-separate border-spacing-0 min-w-[1200px]">
                ...
                <tr>
                  ...
                  <th className="px-10 py-10 border-b border-slate-100">
                    <div className="flex items-center">
                      ...
                      <Tooltip content="Track the submission and approval of research synopsis (Semester 3 milestone)." />
                    </div>
                  </th>
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedStudents.map(student => (
                  <DesktopRow key={student.id} student={student} pendingChanges={pendingChanges} onStatusChange={handleStatusChange} onDateChange={handleDateChange} onCommit={commitSubmission} currentRole={currentRole} />
                ))}
              </tbody>
            </table>
          </div>
        ...
        {filtered.length === 0 && (
          <div className="py-32 text-center bg-white rounded-xl border border-slate-100 shadow-sm">
             <div className="p-10 bg-slate-50 rounded-xl w-fit mx-auto text-slate-200 mb-6"><Search size={60}/></div>
        ...
        {filtered.length > 0 && (
          <div className="mt-8 px-6 md:px-10 py-6 bg-white rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 no-print">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              Showing <span className="text-slate-900">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-slate-900">{Math.min(currentPage * itemsPerPage, filtered.length)}</span> of <span className="text-slate-900">{filtered.length}</span> Candidates
            </div>
...
const DesktopRow = ({ student, pendingChanges, onStatusChange, onDateChange, onCommit, currentRole }: any) => {
...
const FilterDropdown = ({ label, value, icon: Icon, options, displayOptions, onChange }: any) => (
  <div className="space-y-3">
    ...
    <div className="relative group">
      ...
      <select 
        className={`w-full pl-14 pr-12 py-5 bg-white border rounded-xl text-[11px] font-black uppercase tracking-widest outline-none transition-all appearance-none cursor-pointer focus:border-indigo-600 ${value ? 'border-indigo-600 text-indigo-600' : 'border-slate-100 text-slate-500'}`}
        value={value || ''} 
